...
#  })
  ...
  # overall volume
#  depthvoldata = reactive(summarize(group_by(griddata(), depth.qual), 
#    volume = sum(volume)))
#  vol.depth = reactive(ggplot(depthvoldata(), aes(x = depth.qual, y = volume, 
#    fill = depth.qual)) + geom_bar(stat = "identity") + 
#    scale_fill_manual("Depth Habitat", values = depth.colors, 
#      drop = FALSE) + cat.settings + theme(legend.position = "none") 
#  )  
...
########## main panel ##########
  ...
  overall.gathered = reactive({
    ...
    overall = summarize_by_strata(periodgrids(), stratcols = c("date", "id", 
  ...
  output$period_overall = renderPlot({
    if(input$plot_type == "stacked area"){
      ggplot(overall.gathered(), aes(x = datetime, y = volume, fill = habitat, 
        order = -as.numeric(habitat))) + 
      ...
      ggplot(overall.gathered(), aes(x = factor(datetime), y = volume, 
        fill = habitat, order = -as.numeric(habitat))) + xlab("") +
  ...
  bydepth.gathered = reactive({
    hablevels = levels(periodgrids()$habitat)
    bydepth = summarize_by_strata(periodgrids(), stratcols = c("date", "id", 
  ...
  output$period_bydepth = renderPlot({
    if(input$plot_type == "stacked area"){
      ggplot(bydepth.gathered(), aes(x = datetime, y = volume, fill = habitat,
        order = -as.numeric(habitat))) + 
        geom_area(position = "stack") + scale_x_datetime("") +
        scale_fill_manual("", values = habitat.colors, drop = FALSE) +
      ...
      ggplot(bydepth.gathered(), aes(x = factor(datetime), y = volume, 
        fill = habitat, order = -as.numeric(habitat))) + xlab("") +
        geom_bar(stat = "identity", position = "stack") +
        scale_fill_manual("", values = habitat.colors, drop = FALSE) +
        theme(legend.position = "left") +
