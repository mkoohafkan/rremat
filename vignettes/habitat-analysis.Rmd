...
depth_cat = function(z, maxz){
  ifelse(z < 2 & maxz < 2, "littoral",
  ifelse(z < 2 & maxz >= 2, "surface limnetic",
  ifelse(z >= 2 & z < 5 & maxz >= 2, "subsurface limnetic",
  ifelse(z >= 5 & maxz >= 2, "profundal", NA))))
}
```
...
```{r calc-habitat}
data(grids)
bedgrids = summarize(group_by(grids, date, id, dist), bedelev = min(elev))
habgrids = inner_join(grids, bedgrids, by = c("date", "id", "dist"))
habgrids["depth"] = habgrids$wse - habgrids$elev
habgrids["max.depth"] = habgrids$wse - habgrids$bedelev
habgrids["depth.zone"] = factor(ifelse(habgrids$depth < 2,
  "surface (< 2m below surface)", "subsurface (> 2m below surface)"), 
  levels = c("surface (< 2m below surface)", "subsurface (> 2m below surface)"))
habgrids["depth.qual"] = factor(depth_cat(habgrids$depth, habgrids$max.depth),
  levels = c("littoral", "surface limnetic", "subsurface limnetic", "profundal"))
