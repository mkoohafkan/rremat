...
tot_hab_acclimated = make_hsi(list(ta_hsd, oa_hsd, sa_hsd), function(xs) 
...
tot_hab = make_hsi(list(ta_hsd, oa_hsd, sa_hsd), function(xs) 
  ifelse(any(xs == "unsuitable") | any(xs == "acclimated.only"), "unsuitable", 
    ifelse(all(xs %in% c("optimal", "no.impairment", "isotonic")), "optimal", 
    "acceptable")))
```
...
```{r calc-habitat}
safe_mld = function(...) tryCatch(mld(...), error = function(e) NA)
data(grids)
pycnogrids = summarize(group_by(grids, date, id, dist), 
  sa.cline = safe_mld(elev, sa, min.range = 5, max.z = 4))
habgrids = inner_join(grids, pycnogrids, by = c("date", "id", "dist"))
...
habgrids["habitat.acclimated"] = apply(grids[c("ta", "oa", "sa")], 1, 
  tot_hab_acclimated)
```
...
# stratify  by pycnocline
habgrids["pycnozone"] = "below"
habgrids[is.na(habgrids$sa.cline) | habgrids$sa.cline < habgrids$elev, 
  "pycnozone"] = "above"
habgrids["pycnozone"] = factor(habgrids$pycnozone)

# summarize by strata
byelev = summarize_by_strata(habgrids, stratcols = c("date", "id","code", 
  "elevzone", "habitat"), summexpr = c(volume = ~sum(volume)))
bydist = summarize_by_strata(habgrids, stratcols = c("date", "id", "code",
  "distzone", "habitat"), summexpr = c(volume = ~sum(volume)))
bypycnocline = summarize_by_strata(habgrids, stratcols = c("date", "id", 
  "code","pycnozone", "habitat"), summexpr = c(volume = ~sum(volume)))
```
