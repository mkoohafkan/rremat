...
bathytables = dir(volpath, pattern = "*.csv$")
bathydat = read_bathymetry_tables(bathytables)
# build the habitat table
lookupvalues = setNames(expand.grid(wses, wses), c("elev", "wse"))
lookupvalues = subset(lookupvalues, elev <= wse)
volumes = evalmany(f, e = params$elev, w = params$wse, method = "set", 
  default.args = list(d = dat), packages = "dplyr", clusters = 3)[1:9]
