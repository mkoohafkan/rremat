...
volumeres = read.csv(paste0(volpath, "/volumeres.csv"))
attr(volumes, "resolution") = volumeres
```

The hypsometry is generated from the volume lookup table.

```{r}
hypsometry = data.frame(summarize(group_by(volumes, elev), 
  area = sum(count)*prod(attr(volumes, "resolution")[c("xres", "yres")])))
hypsometry["volume"] = cumsum(hypsometry$area*attr(volumes, "resolution")$zres)
attr(hypsometry, "units") = data.frame(area = "square meters", 
  volume = "cubic meters")
```
...
ctdmeta = data.frame(id = factor(ctdm$id), 
  start = as.POSIXct(paste(ctdm$date, ctdm$start), tz = "US/Pacific"), # format = "%Y-%m-%d %H:%M"
...
function `correct_dist` (see `matlab-src/interpctd.m`).
...
Volume information and water-surface elevation is added to the gridded data 
...
```{r strip-grid}
grids = join_wse(join_volume(grids, volumes), ctd)
bathymask = grids$volume > 0
wsemask = grids$elev <= grids$wse
grids = filter(grids[c("date", "id", "dist", "wse", "elev", "sa", "ta", "oa",
  "volume")], volume > 0 & elev <= wse)
```
